...
def dfs(u, graph, discovery, low, parent, time, bridges):
    ...
    for v in graph[u]:
        if discovery[v] == -1:  # v is not visited
            parent[v] = u
            dfs(v, graph, discovery, low, parent, time, bridges)
            ...
            if low[v] > discovery[u]:
                bridges.append((u, v))  # (u, v) is cut edge
        elif v != parent[u]:  # v has been visited and not u's parent node
            low[u] = min(low[u], discovery[v])


def find_bridges(graph):
    num_nodes = len(graph)
    discovery = [-1] * num_nodes  # log discovery time of each node
    low = [-1] * num_nodes  # log minimal reaching time of each node
    parent = [-1] * num_nodes  # parent node list
    time = 0
    bridges = []  # to store cut edges
    for u in range(num_nodes):
        if discovery[u] == -1:
            dfs(u, graph, discovery, low, parent, time, bridges)
    return bridges
...
def opf_setup(ppc, ppopt):
    ...
    contingency_aware = ppopt['CONTINGENCY_AWARE']      ## 1 = N-1 Contingency-Aware OPF, 0 = vanilla OPF
    ...
    if contingency_aware:
        graph = {}  # adjacent list
        for bus_i in bus[:, BUS_I].tolist():
            graph[int(bus_i)] = []
        for (f_bus, t_bus) in zip(branch[:, F_BUS].tolist(), branch[:, T_BUS].tolist()):
            graph[int(f_bus)].append(int(t_bus))
            graph[int(t_bus)].append(int(f_bus))
        cut_edges = find_bridges(graph)
        branches = [branch]     # consider N-|cut_edges| N-1 scenarios and 1 normal scenario
        for i, (f_bus, t_bus) in enumerate(zip(branch[:, F_BUS].tolist(), branch[:, T_BUS].tolist())):
            if (int(f_bus), int(t_bus)) not in cut_edges:
                if (int(t_bus), int(f_bus)) not in cut_edges:
                    branch_temp = copy.deepcopy(branch)
                    branch_temp[i, BR_STATUS] = 0
                    branches.append(branch_temp)
    else:
        branches = [branch]
    ...
    if dc:               ## DC model
        ...
        for branch in branches:
            ...
            Bs.append(B)
            Bfs.append(Bf)
            Pbusinjs.append(Pbusinj)
            Pfinjs.append(Pfinj)
            Amises.append(Amis)
            bmises.append(bmis)

            ## branch flow constraints
            il = find((branch[:, RATE_A] != 0) & (branch[:, RATE_A] < 1e10) & (branch[:, BR_STATUS] == 1))
    ...
    if dc:
        ...
        for i, (Bf, Pfinj, iang, Amis, bmis, il, lpf, upf, upt, Aang, lang, uang) in enumerate(zip(Bfs, Pfinjs, iangs, Amises, bmises, ils, lpfs, upfs, upts, Aangs, langs, uangs)):
            if i == 0:
                om.userdata(f'Bf', Bf)
                om.userdata(f'Pfinj', Pfinj)
                om.userdata(f'iang', iang)
                om.add_constraints(f'Pmis', Amis, bmis, bmis, ['Va', 'Pg'])  ## nb
                om.add_constraints(f'ang', Aang, lang, uang, ['Va'])             ## nang
                om.add_constraints(f'Pf', Bf[il, :], lpf, upf, ['Va'])  ## nl
                om.add_constraints(f'Pt', -Bf[il, :], lpf, upt, ['Va'])  ## nl
            else:
                om.userdata(f'Bf{i}', Bf)
                om.userdata(f'Pfinj{i}', Pfinj)
                om.userdata(f'iang{i}', iang)
                # om.add_constraints(f'ang{i}', Aang, lang, uang, ['Va'])             ## nang
                om.add_constraints(f'Pf{i}',  Bf[il, :], lpf, upf, ['Va'])          ## nl
                om.add_constraints(f'Pt{i}', -Bf[il, :], lpf, upt, ['Va'])          ## nl
    else:
        ...
        for i, (iang, Aang, lang, uang) in enumerate(zip(iangs, Aangs, langs, uangs)):
            om.userdata(f'iang{i}', iang)
            om.add_constraints(f'ang{i}', Aang, lang, uang, ['Va'])        ## nang
