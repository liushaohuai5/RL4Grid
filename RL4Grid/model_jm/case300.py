...
def case300():
    ...
    min_k_elements_with_index = heapq.nsmallest(21, enumerate(ppc['gen'][:, PMAX].tolist()), key=lambda x: x[1])
    ppc['renewable_ids'] = [index for index, value in min_k_elements_with_index]
    ppc['thermal_ids'] = [i for i in range(ppc['num_gen'])]
    ppc['thermal_ids'].remove(ppc['balanced_id'])
    for i in ppc['renewable_ids']:
        ppc['thermal_ids'].remove(i)
