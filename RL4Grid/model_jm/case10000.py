...
def case10000():
    ...
    ppc["bus"] = np.load(path+'WE10000_bus.npy')

    ## generator data
    # bus, Pg, Qg, Qmax, Qmin, Vg, mBase, status, Pmax, Pmin, Pc1, Pc2,
    # Qc1min, Qc1max, Qc2min, Qc2max, ramp_agc, ramp_10, ramp_30, ramp_q, apf
    ppc["gen"] = np.load(path+'WE10000_gen.npy')
    # ppc["gen"][39, GEN_BUS] = 1091


    ## branch data
    # fbus, tbus, r, x, b, rateA, rateB, rateC, ratio, angle, status, angmin, angmax
    ppc["branch"] = np.load(path+'WE10000_branch.npy')

    ##-----  OPF Data  -----##
    ## generator cost data
    # 1 startup shutdown n x1 y1 ... xn yn
    # 2 startup shutdown n c(n-1) ... c0
    ppc["gencost"] = np.load(path+'WE10000_gencost.npy')

    ppc['coordinates'] = np.load(path+'WE10000_coordinates.npy')

    # ppc['bus'][:, BUS_AREA] = 1
    # ppc['branch'][:, [BR_R, BR_X, BR_B]] = np.abs(ppc['branch'][:, [BR_R, BR_X, BR_B]])
    # ppc['branch'][:, BR_X] += 0.1
    # ppc['branch'][:, BR_X] = ppc['branch'][:, BR_X].clip(1e-3, 1000)
    # ppc['branch'][:, BR_B] /= 10
    # ppc['branch'][:, ANGMIN] = -360.0
    # ppc['branch'][:, ANGMAX] = 360.0
    # ppc['branch'][:, [RATE_A, RATE_B, RATE_C]] = ppc['branch'][:, [RATE_A, RATE_B, RATE_C]].clip(3e2, 1e8)
    # ppc['branch'][:, [RATE_A, RATE_B, RATE_C]] = 10000.0
    # ppc['gencost'][:, 4] += 0.001
    # overload_lines = [438]
    # ppc['branch'][overload_lines, [RATE_A]] *= 5
    sensitive_line_idxs = [2213, 2720, 2760, 2789, 2790, 2806, 2952, 4279, 4292, 4466, 4467, 5815, 5823, 5837, 5855, 5856, 5928, 5956, 5964, 6016, 6017, 6127, 7185, 7186, 7198, 7204, 7205, 7312, 7313, 7335, 7344, 7371, 7372, 7406, 7829, 7890, 9087, 9122, 9155, 9191, 9219, 9263, 9273, 9324, 9400, 9408, 9473, 9940, 10267, 10268, 10269, 10318, 10319, 10327, 11469, 11470, 11509, 11510, 11570, 11571, 11827, 11841, 11842, 11843, 12131, 12175, 12213, 12219, 12245, 12246, 12506, 12550, 12551]
    suggested_capacities = [12, 27, 33, 20, 31, 20, 19, 26, 48, 58, 58, 74, 34, 32, 69, 38, 38, 65, 61, 34, 60, 50, 42, 65, 57, 74, 37, 37, 37, 78, 80, 74, 74, 82, 38, 38, 40, 40, 83, 40, 39, 38, 47, 48, 39, 39, 97, 38, 77, 65, 64, 79, 64, 55, 69, 35, 75, 61, 76, 74, 40, 54, 96, 87, 72, 33, 20, 93, 45, 43, 57, 34, 34]
    for i, idx in enumerate(sensitive_line_idxs):
        ppc['branch'][idx, [RATE_A, RATE_B, RATE_C]] = suggested_capacities[i]

    ppc['network'] = 'Western10000'
    ...
    ppc['gen_type'] = np.load(path+'WE10000_gen_type.npy')[:ppc['num_gen']]
    ...
    # ppc['gencost'][ppc['renewable_ids'], 4] = 0.0
    # ppc['gencost'][ppc['renewable_ids'], 5] = 0.0
