...
def check_gen_p(adjust_gen_p, action_space_gen_p, gen_ids, eps):
    ...
    illegal_gen_ids = []
    for i in gen_ids:
        if adjust_gen_p[i] <= action_space_gen_p.low[i] - eps or adjust_gen_p[i] >= action_space_gen_p.high[i] + eps:
            illegal_gen_ids.append(i)
    return illegal_gen_ids

def check_gen_v(adjust_gen_v, action_space_gen_v, gen_ids, eps):
    illegal_gen_ids = []
    for i in gen_ids:
        if adjust_gen_v[i] < action_space_gen_v.low[i] - eps or adjust_gen_v[i] > action_space_gen_v.high[i] + eps:
            illegal_gen_ids.append(i)
    return illegal_gen_ids

def is_legal(act, last_obs, ppc):
    """
    Returns:
        illegal_reasons(list): reasons why the action is illegal 
    """
    gen_ids = list(range(ppc['num_gen']))
    illegal_reasons = []
