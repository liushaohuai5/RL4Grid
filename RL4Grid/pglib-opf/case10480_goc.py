...
def case10480_goc():
    ...
    ppc["bus"] = np.load(path+"case10480_goc_bus.npy")

    ## generator data
    # bus, Pg, Qg, Qmax, Qmin, Vg, mBase, status, Pmax, Pmin, Pc1, Pc2,
    # Qc1min, Qc1max, Qc2min, Qc2max, ramp_agc, ramp_10, ramp_30, ramp_q, apf
    ppc["gen"] = np.load(path+"case10480_goc_gen.npy")


    ## branch data
    # fbus, tbus, r, x, b, rateA, rateB, rateC, ratio, angle, status, angmin, angmax
    ppc["branch"] = np.load(path+"case10480_goc_branch.npy")

    ##-----  OPF Data  -----##
    ## generator cost data
    # 1 startup shutdown n x1 y1 ... xn yn
    # 2 startup shutdown n c(n-1) ... c0
    ppc["gencost"] = np.load(path+"case10480_goc_gencost.npy")

    # ppc['branch'][:, [RATE_A, RATE_B, RATE_C]] = ppc['branch'][:, [RATE_A, RATE_B, RATE_C]].clip(50, 1e8)
    sensitive_line_idxs = [6]
    suggested_capacities = [800]
    for i, idx in enumerate(sensitive_line_idxs):
        ppc['branch'][idx, [RATE_A, RATE_B, RATE_C]] = suggested_capacities[i]

    ppc["network"] = "case10480_goc"
